...
function getValue(row: ExportRow, names: string[]): any {
...
function getNumber(row: ExportRow, names: string[]): number {
  return num(getValue(row, names));
}

function sumAnyCols(row: ExportRow, namesA: string[], namesB: string[]) {
  return getNumber(row, namesA) + getNumber(row, namesB);
}
...
function isPeriodColumn(col: string) {
  return col.endsWith(" Período") || col.endsWith(" Período (h)");
}

function cleanReasonRoot(col: string) {
  return col.replace(" Período (h)", "").replace(" Período", "").trim();
}
...
function getReasonRoots(rows: ExportRow[]) {
  ...
  const periodCols = cols.filter(isPeriodColumn);
...
function getReasonTotal(row: ExportRow, root: string) {
  return (
    getNumber(row, [`${root} Período`, `${root} Período (h)`]) +
    getNumber(row, [`${root} Desligado`, `${root} Desligado (h)`])
  );
}
...
function groupByMachine(rows: ExportRow[]): MachineAgg[] {
  ...
  for (const r of rows) {
    const m = String(getValue(r, ["Máquina", "Maquina", "Machine"]) ?? "").trim();
    if (!m) continue;
  ...
  for (const [maquina, items] of map.entries()) {
    const validTrab = items.filter((x) => getValue(x, ["Utilização Trabalhando", "Utilização Trabalho (%)"]) !== 0);
    const validTransp = items.filter((x) => getValue(x, ["Utilização Transporte", "Utilização Transporte (%)"]) !== 0);
    const validOcioso = items.filter((x) => getValue(x, ["Utilização Ocioso", "Utilização Ocioso (%)"]) !== 0);

    const trabAvg =
      (validTrab.length ? validTrab : items).reduce(
        (acc, x) => acc + pct01(getValue(x, ["Utilização Trabalhando", "Utilização Trabalho (%)"])),
        0
      ) / Math.max(1, (validTrab.length ? validTrab : items).length);

    const transpAvg =
      (validTransp.length ? validTransp : items).reduce(
        (acc, x) => acc + pct01(getValue(x, ["Utilização Transporte", "Utilização Transporte (%)"])),
        0
      ) / Math.max(1, (validTransp.length ? validTransp : items).length);

    const ociosoAvg =
      (validOcioso.length ? validOcioso : items).reduce(
        (acc, x) => acc + pct01(getValue(x, ["Utilização Ocioso", "Utilização Ocioso (%)"])),
        0
      ) / Math.max(1, (validOcioso.length ? validOcioso : items).length);
    ...
    const canceladoH = items.reduce(
      (acc, x) =>
        ...
        sumAnyCols(
          x,
          ["Cancelado Período", "Cancelado Período (h)"],
          ["Cancelado Desligado", "Cancelado Desligado (h)"]
        ),
      0
    );
    ...
    const ignoradoH = items.reduce(
      (acc, x) =>
        ...
        sumAnyCols(
          x,
          ["Ignorado Período", "Ignorado Período (h)"],
          ["Ignorado Desligado", "Ignorado Desligado (h)"]
        ),
      0
    );
    ...
    const indefinidoH = items.reduce(
      (acc, x) =>
        ...
        sumAnyCols(
          x,
          ["Indefinido Período", "Indefinido Período (h)"],
          ["Indefinido Desligado", "Indefinido Desligado (h)"]
        ),
      0
    );
...
function buildTop3ByMachine(rows: ExportRow[]) {
  ...
  for (const r of rows) {
    const maquina = String(getValue(r, ["Máquina", "Maquina", "Machine"]) ?? "").trim();
    if (!maquina) continue;
...
export default function App() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const first = json[0] ?? {};
      const hasMachine = "Máquina" in first || "Maquina" in first || "Machine" in first;
      const hasWork = "Utilização Trabalhando" in first || "Utilização Trabalho (%)" in first;
      const hasTransport = "Utilização Transporte" in first || "Utilização Transporte (%)" in first;
      const hasIdle = "Utilização Ocioso" in first || "Utilização Ocioso (%)" in first;

      const missing: string[] = [];
      if (!hasMachine) missing.push("Máquina");
      if (!hasWork) missing.push("Utilização Trabalhando / Utilização Trabalho (%)");
      if (!hasTransport) missing.push("Utilização Transporte");
      if (!hasIdle) missing.push("Utilização Ocioso");
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => String(getValue(r, ["Máquina", "Maquina", "Machine"]) ?? "").trim() === machineFilter);
  }, [rows, machineFilter]);
